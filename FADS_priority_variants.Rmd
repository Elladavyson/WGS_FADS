...
for (i in c(1:nrow(FADS))){
  ...
  priority_mdd <- read.table(paste0("pri_variant_files/", gene, "_priority_var_mdd.tsv"), sep = "\t", header =T)
  priority_metabol <- read.table(paste0("pri_variant_files/", gene, "_priority_var_metabol.tsv"), sep = "\t", header =T)
...
pri_csq_mdd_all <- rbind(FADS1_priority_csq_mdd, FADS2_priority_csq_mdd, FADS3_priority_csq_mdd, FEN1_priority_csq_mdd, MYRF_priority_csq_mdd, TMEM258_priority_csq_mdd) %>% mutate(cohort = paste0(cohort, " cohort"))

pri_csq_metabol_all <- rbind(FADS1_priority_csq_metabol, FADS2_priority_csq_metabol, FADS3_priority_csq_metabol, FEN1_priority_csq_metabol, MYRF_priority_csq_metabol, TMEM258_priority_csq_metabol) %>% mutate(cohort = paste0(cohort, " cohort"))
...
mdd_cohort_priority <- ggplot(pri_csq_mdd_all, aes(x = reorder(Consequence, csq_count), y = csq_count, fill = reorder(IMPACT, csq_count))) +
...
metabolite_cohort_priority <- ggplot(pri_csq_metabol_all, aes(x = reorder(Consequence, csq_count), y = csq_count, fill = reorder(IMPACT, csq_count))) +
  geom_bar(stat = 'identity') + 
 theme_bw() + 
labs(x = "Consequence", y = "N", title = "Prioritised variants in the MDD cohort", 
fill = "VEP IMPACT") +
...
all_cohort_pri_csq <- ggplot(pri_csq_all, aes(x = reorder(Consequence, csq_count), y = csq_count, fill = reorder(IMPACT, csq_count))) +
  geom_bar(stat = 'identity') + 
 theme_bw() + 
...
summarise_mask_counts <- function(dataframe, cohortlab) {
  ...
  cohort = paste0(cohortlab, "_cohort"),
  AF_Threshold = as.factor(AF_Threshold)
    ) %>% 
...
mask_af_plt <- ggplot(mask_af_long %>% filter(Value ==1 & (AF_Threshold == 'singleton' | AF_Threshold == "0.01")), aes(x = AF_Threshold, fill = mask_label)) +
