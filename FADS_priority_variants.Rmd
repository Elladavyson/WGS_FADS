...
ggsave(filename= "/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Plots/FADS_pri_variants_subplots.png", ggarrange(all_cohort_pri_csq, ACcounts_graph, ncol = 1, nrow = 2), width = 14, height = 14, device = "png", dpi= 300)
...
if (!("FADS1_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS1_priority_MDD_cohort")
}
writeData(supp_wb,"FADS1_priority_MDD_cohort", FADS1_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("FADS1_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS1_priority_Meta_cohort")
}
writeData(supp_wb,"FADS1_priority_Meta_cohort", FADS1_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))

# FADS2 priority variants 
if (!("FADS2_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS2_priority_MDD_cohort")
}
writeData(supp_wb,"FADS2_priority_MDD_cohort", FADS2_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("FADS2_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS2_priority_Meta_cohort")
}
writeData(supp_wb,"FADS2_priority_Meta_cohort", FADS2_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))

# FADS3 priority variants 

if (!("FADS3_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS3_priority_MDD_cohort")
}
writeData(supp_wb,"FADS3_priority_MDD_cohort", FADS3_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("FADS3_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FADS3_priority_Meta_cohort")
}
writeData(supp_wb,"FADS3_priority_Meta_cohort", FADS3_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))

# FEN1 priority variants 
if (!("FEN1_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FEN1_priority_MDD_cohort")
}
writeData(supp_wb,"FEN1_priority_MDD_cohort", FEN1_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("FEN1_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "FEN1_priority_Meta_cohort")
}
writeData(supp_wb,"FEN1_priority_Meta_cohort", FEN1_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))

# MYRF priority variants 
if (!("MYRF_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "MYRF_priority_MDD_cohort")
}
writeData(supp_wb,"MYRF_priority_MDD_cohort", MYRF_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("MYRF_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "MYRF_priority_Meta_cohort")
}
writeData(supp_wb,"MYRF_priority_Meta_cohort", MYRF_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))

# TMEM258 priority variants 
if (!("TMEM258_priority_MDD_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "TMEM258_priority_MDD_cohort")
}
writeData(supp_wb,"TMEM258_priority_MDD_cohort", TMEM258_priority_mdd %>% 
            select(-ends_with("_cov"), FILTER.y))

# Metabolite cohort 
if (!("TMEM258_priority_Meta_cohort" %in% sheet_names)) {
  addWorksheet(supp_wb, "TMEM258_priority_Meta_cohort")
}
writeData(supp_wb,"TMEM258_priority_Meta_cohort", TMEM258_priority_metabol %>% 
            select(-ends_with("_cov"), FILTER.y))
