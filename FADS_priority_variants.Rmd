...
```{r}
####### Consequence plots 
pri_csq_mdd_all <- rbind(FADS1_priority_csq_mdd, FADS2_priority_csq_mdd, FADS3_priority_csq_mdd, FEN1_priority_csq_mdd, MYRF_priority_csq_mdd, TMEM258_priority_csq_mdd) %>% mutate(cohort = paste0(cohort, "-cohort"), IMPACT = factor(IMPACT, levels = c("HIGH", "MODERATE", "MODIFIER", "LOW")))

pri_csq_metabol_all <- rbind(FADS1_priority_csq_metabol, FADS2_priority_csq_metabol, FADS3_priority_csq_metabol, FEN1_priority_csq_metabol, MYRF_priority_csq_metabol, TMEM258_priority_csq_metabol) %>% mutate(cohort = paste0(cohort, "-cohort"), IMPACT = factor(IMPACT, levels = c("HIGH", "MODERATE", "MODIFIER", "LOW")))
...
all_cohort_pri_csq <- ggplot(pri_csq_all, aes(x = reorder(Consequence, csq_count), y = csq_count, fill = IMPACT)) +
  geom_bar(stat = 'identity') + 
 theme_bw() + 
labs(x = "Consequence", y = "N", title = "Prioritised variants in the MDD and Metabolite cohort", 
fill = "VEP IMPACT") +
coord_flip() + 
geom_text(aes(label = csq_count), hjust = -0.1) + 
 theme(legend.position = "top",
...
ggsave(filename= "/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Plots/FADS_pri_variants_csq_allgenes.png", all_cohort_pri_csq, width = 12, height = 10, device = "png", dpi= 300)
...
masks_bothcohorts <- ggplot(maskcounts_all, aes(x = AF_Threshold, y= count, fill = mask_label_factor)) +
 ...
 geom_text(stat="identity", aes(label = count, color = mask_label_factor), vjust = -0.5, size = 3, show.legend = FALSE, position = position_dodge(width = 0.8)) +
...
masks_bothcohorts, width = 12, height = 6, device = "png", dpi= 300)
masks_bothcohorts
...
priority_gene <- priority_var_all %>% filter(SYMBOL == gene)
  if(cohort == "MDD") {
    mdd_variants <- read.table(paste0("/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Intermediate_data_files/pri_variant_files/mdd_metabol_cohortvars/", gene, "_priority_var_mdd.tsv"), sep = "\t", header=T)
    priority_gene <- priority_gene %>% filter(chr_pos_ref_alt %in% mdd_variants$chr_pos_ref_alt)
  }
else if(cohort == "Metabolite") {
  metabol_variants <- read.table(paste0("/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Intermediate_data_files/pri_variant_files/mdd_metabol_cohortvars/", gene, "_priority_var_metabol.tsv"), sep = "\t", header=T)
  priority_gene <- priority_gene %>% filter(chr_pos_ref_alt %in% metabol_variants$chr_pos_ref_alt)
...
print(paste0("Saving the lolliplot of Mask", j, ".0.01 for", gene, "to ", gene, "_Mask", j, ".0.01_lolliplot_variants_pri_", cohort, ".png"))
png(paste0("/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Plots/", gene, "_Mask", j, ".0.01_lolliplot_variants_pri_", cohort, ".png"),
...
print(paste0("Saving the lolliplot of Mask", j, ".singleton for", gene, "to ", gene, "_Mask", j, ".singleton_lolliplot_variants_pri_", cohort, ".png"))
png(paste0("/Users/ellad/UniversityEdinburgh/PhD/Year_3/WGS_proj/Plots/", gene, "_Mask", j, ".singleton_lolliplot_variants_pri_", cohort, ".png"),
    width = 4000, height = 1200, res = 300, type = "cairo")
...
dev.off()
  ...
  print(paste0("All lolliplots for each gene and Mask for the", cohort, "-cohort"))
}

lolliplots("MDD")
lolliplots("Metabolite")
```
...
ggplot(mask4_0.01, aes(x = as.factor(AC), fill = as.factor(most_deleterious))) + geom_bar(stat="count") + facet_wrap(~inlovo)
