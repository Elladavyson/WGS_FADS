...
genes <- c("FADS1", "FADS3", "FEN1", "MYRF", "TMEM258")
for (i in genes) {
    print(i)
vepoutput <- read.table(paste0(i, "_vepoutput_proc/", i, "_priority_annot.tsv"),
sep = '\t', header = T)
vepoutput$GENE = i
...
ranges <- sapply(list(FADS1_vep_pri, FADS3_vep_pri, FEN1_vep_pri, 
                      MYRF_vep_pri, TMEM258_vep_pri), count_af_ranges)

# Create a data frame with results
af_counts <- data.frame(
  Gene = genes,
  Range1_AF_less_0.000001 = ranges[1, ],
  Range2_AF_0.000001_to_0.00001 = ranges[2, ],
  Range3_AF_0.00001_to_0.0001 = ranges[3, ],
Range4_AF_0.0001_to_0.001 = ranges[4, ],
Range5_AF_0.001_to_0.01 = ranges[5, ]
)

# View the final table
print(af_counts)

n_variants <- data.frame(Gene = genes, All = c(nrow(FADS1_vep_pri), nrow(FADS3_vep_pri), 
...
long_data <- pivot_longer(
  ...
  names_to = "AF_Range",        # New column for the range labels
...
plot_af <- ggplot(long_data, aes(x = AF_Range, y = Variant_Count, fill = AF_Range)) +
